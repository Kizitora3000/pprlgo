	...
	*/
	...
	for i := 0; i < Nv; i++ {
		...
		if v_t[i] == 0 {
			...
			doublenc.DEencBFV(params, encoder, encryptor, publicKey, zeros, filename)
		} else if v_t[i] == 1 {
			ones := make([]uint64, Na)
			for i := range ones {
				ones[i] = 1
			}
			doublenc.DEencBFV(params, encoder, encryptor, publicKey, ones, filename)
		}
	}

	doublenc.DEencBFV(params, encoder, encryptor, publicKey, w_t, WtName)
	...
	doublenc.DEencBFV(params, encoder, encryptor, publicKey, Q_news, Q_news_name)
	...
	for i := 0; i < Nv; i++ {
		...
		v_and_w_Qnew := make([]uint64, Na)
		fhe_v_and_w_Qnew := doublenc.BFVenc(params, encoder, encryptor, v_and_w_Qnew)
		fhe_v_and_w_Qnew = evaluator.MulNew(fhe_v_t, fhe_w_t)
		evaluator.Relinearize(fhe_v_and_w_Qnew, fhe_v_and_w_Qnew)
		fhe_v_and_w_Qnew = evaluator.MulNew(fhe_v_and_w_Qnew, fhe_Q_news)

		// make Qold
		v_and_w_Qold := make([]uint64, Na)
		fhe_v_and_w_Qold := doublenc.BFVenc(params, encoder, encryptor, v_and_w_Qold)
